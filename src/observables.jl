...
@doc """
    ...
    """
function frequency_exceedance(sol, filter, threshold, n_days)
    # Using views is important, otherwise every instance of sol[filter,i]
    # allocates a new temporary array.
    start_t = sol.prob.tspan[2] - floor(n_days * 24 * 3600)
    start_i = findfirst(t -> t > start_t, sol.t)
    ...
    @views begin
        ...
        if any(x -> abs(x) > threshold, sol[filter, start_i])
            # The solution already starts in an exceedance region
            in_ex_region = true
            # we average the neighbouring time steps, /2 is at the end
            ex_t -= sol.t[start_i] + sol.t[start_i]
        end

        for i = (start_i+1):length(sol.t)
            step_in_region = any(x -> abs(x) > threshold, sol[filter, i])
...
function sum_abs_energy_last_days(sol, energy_filter, n_days)
    ...
    total .= 0.0
    for t = (t_end-floor(n_days * 24 * 3600)):t_end
...
function var_last_days(sol, state_filter, n_days)
    t_end = sol.prob.tspan[2]
    tail = [sol(t)[state_filter] for t = (t_end-n_days*24*3600+1):t_end]
    var(tail)
end
...
function energy_cost(sol, energy_filter, energy_abs_filter, num_days, obs_days)
    ...
    sum_lambda = 3600.0 * (sol.prob.p.hl.lambda - 1)
    ...
    sign_term = sol.prob.p.hl.lambda .* sum_sign .+ sum_lambda
    ...
    cost = (1 - sol.prob.p.hl.lambda) .* sum(
        abs.(ILC_power_hourly[24*(num_days-obs_days):24*num_days, :]),
        dims = 1,
    ) .+
           sol.prob.p.hl.lambda .* sum(
