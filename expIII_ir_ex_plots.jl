...
for m = 1:length(ilc_lst) # ilc_lst = 1:N
    ...
    Ex_all_nodes_III = DataFrame(Experiment = m, Result = [p[2] for p in res_tlIII_ir.u][(m-1)*batch_size+1:m*batch_size] * 100. / (3600 * 24 * obs_days)) # local ilc at m random nodes
