...
_compound_pars = experiments.compound_pars(N, low_layer_control, kappa4, vc4, cover4, lambda)
...
@everywhere begin
	...
	ode_tlIV = ODEProblem(network_dynamics.ACtoymodel!, ic, tspan, compound_pars,
		callback=CallbackSet(PeriodicCallback(network_dynamics.HourlyUpdateEcon(), l_hour, initial_affect= false),
						 PeriodicCallback(network_dynamics.DailyUpdate_IV, l_day, initial_affect= false)))
end

#sol = solve(ode_tlIV, Rodas4P())

monte_probIV = EnsembleProblem(
	ode_tlIV,
	output_func = (sol, i) -> experiments.observer_basic_types_lambda_IV(sol, i, freq_filter, energy_filter,  energy_abs_filter, freq_threshold, obs_days),
	prob_func = (prob,i,repeat) -> experiments.prob_func_IV(prob,i,repeat, batch_size, graph_lst, num_days, lambda_lst),
	u_init = [])

res_tlIV = @time solve(monte_probIV,
					 Rodas4P(),
...
jldopen("$dir/solutions/$(date)/expIV_sol_new.jld2", true, true, true, IOStream) do file
	file["monte_probIV"] = monte_probIV
	file["res_tlIV"] = res_tlIV
end

graph_dict = Dict("g$i"=> Graph(res_tlIV.u[i][5]) for i = 1:length(res_tlIV))
savegraph("$dir/solutions/$(date)/expIV_graph_lst.lg", graph_dict)
