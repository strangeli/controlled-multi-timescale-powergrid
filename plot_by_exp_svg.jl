...
plot!(collect(0.2:0.1:1), y2, label = "II:  ILC at ind. set + neigh. com.", lw = 3, lt=:scatter, m = :cross, color = :green, alpha = 0.4)
plot!(collect(0.2:0.1:1), y3, label = "", ls = :dash, lw=3,  color = :purple, alpha = 0.4)
plot!(collect(0.2:0.1:1), y3, label = "III: local ILC at ind. set", ls = :dash, lw=3,  lt=:scatter, m = :hex, color = :purple, alpha = 0.4)
plot!(collect(0.2:0.1:1), y4, label = "", ls=:dashdot, lw = 3, color = :orange, alpha = 0.4)
plot!(collect(0.2:0.1:1), y4, label = "IV: ILC rand. 50% + rand. com.", ls=:dashdot, lw = 3, m = :star7, color = :orange,lt=:scatter, alpha = 0.4)
xlabel!(L"\lambda [-]")
