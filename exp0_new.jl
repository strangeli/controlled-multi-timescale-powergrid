...
vc0 = [] # ilc_nodes (here: without communication)
cover0 = Dict([v => [] for v in vc0])# ilc_cover
lambda = lambda_lst[1]

_compound_pars = experiments.compound_pars(N, low_layer_control, kappa, vc0, cover0, lambda)
...
monte_prob0 = EnsembleProblem(
	...
	prob_func = (prob,i,repeat) -> experiments.prob_func_0(prob, i, repeat, batch_size, graph_lst, num_days, lambda_lst),
	u_init = [])

res_tl0 = @time solve(monte_prob0,
					 Rodas4P(),
...
jldopen("$dir/solutions/$(date)/exp0_sol_new.jld2", true, true, true, IOStream) do file
	file["monte_prob0"] = monte_prob0
	file["res_tl0"] = res_tl0
end

graph_dict = Dict("g$i"=> Graph(res_tl0.u[i][5]) for i = 1:length(res_tl0))
savegraph("$dir/solutions/$(date)/exp0_graph_lst.lg", graph_dict)
